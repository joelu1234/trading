...
public class YahooOptionReceiver {
	...
	private static void fetchOptions(Document doc, Stock stock) {
		...
		{
			boolean isCall = "Calls".equals(table.getElementsByTag("caption").first().text()); //Calls or Puts
	...
	//http://finance.yahoo.com/q/op?s=AAPL&date=1424390400 time is seconds in GMT
	private static String getUrl(String ticker) {
		StringBuilder sb = new StringBuilder(PropertyManager.getInstance()
				.getProperty(PropertyManager.YAHOO_OPTION));
		sb.append(ticker);
		sb.append("&&date=");
